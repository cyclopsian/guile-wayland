...
(test-group "client-server-integration"
  (define (run-server-and-client)
    (match-let* (((server socket-name) (make-server))
                 ...
                 (client-dispatch
                   (λ (mask)
                     (display mask) (newline)
                      (when (positive? (logand mask WL_EVENT_ERROR))
                        (error "client socket error" client))
                      (when (positive? (logand mask WL_EVENT_WRITABLE))
                        (flush client))
                      (when (positive? (logand mask WL_EVENT_READABLE))
                        (while (not (prepare-read client))
